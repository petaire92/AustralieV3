...
class X2ViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    
    // MARK: Variables
    let cellID = "pouet"
    let gfx = Gfx()
    var cell : X2ContentCollectionViewCell?
    ...
    override func viewDidLoad() {
        ...
        mainCollectionView.register(X2ContentCollectionViewCell.self, forCellWithReuseIdentifier: cellID)
        addMenuScrollView()
        addMainView()
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        cell = mainCollectionView.dequeueReusableCell(withReuseIdentifier: cellID, for: indexPath) as? X2ContentCollectionViewCell
        cell?.X1VC.titleView.backgroundColor = gfx.getUiColorArray()[indexPath.item]
        cell?.X1VC.titleLabel.text = gfx.arrayOfMenuText?[indexPath.item]
//        cell?.X1VC.scrollView.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: 100, right: 0)
        return cell!
    }
